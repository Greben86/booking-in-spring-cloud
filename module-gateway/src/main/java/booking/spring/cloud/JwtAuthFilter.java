...
public class JwtAuthFilter extends AbstractGatewayFilterFactory<JwtAuthFilter.Configuration> {
    ...
    private final Cache<String, Key> keyCache;
    ...
    private UserInformation extractToken(String token, @NonNull Key key) {
        final var claimsJws = Jwts.parserBuilder()
                .setSigningKey(key)
                .build()
                .parseClaimsJws(token);
        return new UserInformation(claimsJws.getBody());
    }
        ...
        return (exchange, chain) -> {
            ...
            if (userInformation.isValid()) {
                ServerHttpRequest mutatedRequest = exchange.getRequest()
                        .mutate()
                        .header("X-User-Id", String.valueOf(userInformation.getId()))
                        .header("X-User-Name", userInformation.getLogin())
                        .header("X-User-Role", userInformation.getRole().name())
                        .build();
    ...
    private Key getKey() {
        return keyCache.get("SIGNING_KEY", () -> {
            ResponseEntity<String> key = restTemplate.getForEntity("http://module-booking:8081/key", String.class);
    ...
    private static final class UserInformation {
        ...
        private UserInformation(Claims claims) {
            ...
            this.login = claims.get("login", String.class);
            this.role = claims.get("role", UserRole.class);
        }

        public boolean isValid() {
            return getId() != null && getLogin() != null && getRole() != null;
        }
    }
}